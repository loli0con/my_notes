...
|${basedir}/src/main/webapp|Web应用资源|
...
执行**任务**或**目标**时，Maven会在当前目录中查找POM。它读取POM，获取所需的配置信息，然后执行目标。

### Super POM
所有的POM都隐式继承自一个[super-POM](https://maven.apache.org/ref/3.9.11/maven-model-builder/super-pom.html)，super-POM包含了一些可以被继承的默认设置。

### effective pom
effective POM是一个**完整且最终**的项目构建模型，它被Maven用于构建项目。它通过多层级的配置叠加合并后生成：
1. Maven内部默认值和Super-POM（隐式继承）
2. 全局settings.xml文件（系统级）
3. 用户特定的settings.xml文件（位于~/.m2目录中）
4. 父POM（在项目中显式指定）
5. 项目自身的POM
6. 被激活的Profiles（根据特定条件激活的配置）

在存放pom.xml的路径下，可以使用`mvn help:effective-pom`命令来查看effective POM。
...
* 使用命令向导一步步创建项目
   ...
   2. 打开命令行窗口，切换到该目录
   3. 输入“mvn archetype:generate”，按Enter键
   4. 一步步按照提示输入groupId、artifactId、version、package等信息
* 在命令中输入所有必要信息直接创建项目
   1. 在硬盘上创建一个空的目录，用来存放Maven项目
   2. 打开命令行窗口，切换到该目录
